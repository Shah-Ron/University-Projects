...
class Model:
    ...
    def __init__(self):
        ...
        self.input_size = 784   # Size of image vectors being 28 * 28
        self.num_classes = 10 # 10 possible labels (digits 0-9)
        self.batch_size = 100
    ...
    def call(self, inputs):
        ...
        forward = np.matmul(inputs, self.W) + self.b
        return forward
    ...
    def back_propagation(self, inputs, outputs, labels):
        ...
        # Compute gradients using the Perceptron Learning Algorithm
        predictions = np.argmax(outputs, axis =1)
        predicted_values = np.eye(self.num_classes)[predictions]
        
        expected_values = np.eye(self.num_classes)[np.ravel(labels)]
    
        diff = expected_values - predicted_values
        gradW = np.matmul(np.transpose(diff), inputs) / self.num_classes
        gradB = np.matmul(np.transpose(diff), np.ones((len(inputs), 1))) / self.num_classes
    ...
    def gradient_descent(self, gradW, gradB):
        ...
        self.W += self.learning_rate * np.transpose(gradW)
        self.b += self.learning_rate * np.transpose(gradB)
...
def test(model, test_inputs, test_labels):
    ...
    accuracy = model.accuracy(prediction, test_labels)
    return accuracy
...
def main(mnist_data_folder):
    ...
    # Define file paths
    train_inputs_file_path = f'{mnist_data_folder}/train-images-idx3-ubyte.gz'
    train_labels_file_path = f'{mnist_data_folder}/train-labels-idx1-ubyte.gz'
    test_inputs_file_path = f'{mnist_data_folder}/t10k-images-idx3-ubyte.gz'
    test_labels_file_path = f'{mnist_data_folder}/t10k-labels-idx1-ubyte.gz'

    # Define number of examples
    num_train_examples = 60000
    num_test_examples = 10000

    train_inputs, train_labels = get_data(train_inputs_file_path, train_labels_file_path, num_train_examples)
    test_inputs, test_labels = get_data(test_inputs_file_path, test_labels_file_path, num_test_examples)
    ...
    accuracy = 100 * test(model, test_inputs, test_labels)
    print(f"The Test Accuracy is {accuracy:.2f}%")

    probability = model.call(test_inputs[0:10])

    # TODO: Visualize the data by using visualize_results()
    visualize_results(test_inputs[0:10], probability, test_labels[0:10])
