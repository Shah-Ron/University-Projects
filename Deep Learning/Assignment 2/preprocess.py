...
def pre_process_data(inputs, labels, first_class, second_class):
	...
	# Filter inputs and labels for the first and second classes
	mask = (labels == first_class) | (labels == second_class)
	filtered_inputs = inputs[mask]
	filtered_labels = labels[mask]

	# Reshape inputs to (num_examples, 3, 32, 32)
	reshaped_inputs = np.reshape(filtered_inputs, (-1,3,32,32))
	# Transpose to get the finale shape
	reshaped_inputs = np.transpose( reshaped_inputs, (0, 2, 3, 1))

	# Normalizing the inputs to have the values between 0 and 1
	normalized_inputs = reshaped_inputs.astype(np.float32) / 255.0

	# Convert the labels to binary class labels (0 or 1)
	binary_labels = np.where(filtered_labels == first_class, 0 , 1)

	# Convert the labels to one hot vectors using tf.one_hot
